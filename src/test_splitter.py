...
class TestSplitter(unittest.TestCase):
    ...
    def test_link(self):
        node = TextNode("Text [with a link to boot.dev](https://www.boot.dev) in it!", TextType.TEXT)
    ...
    def test_image(self):
        node = TextNode("Text ![image of a cat](https://cats.com/cat.png) in it!", TextType.TEXT)
        nodes = split_nodes_images([node])
        self.assertEqual(len(nodes), 3)
        self.assertEqual(nodes[0].text, "Text ")
        self.assertEqual(nodes[0].text_type, TextType.TEXT)
    ...
    def test_multiple_links(self):
        node = TextNode("Opening [first](https://first.com) middle [second](https://second.com) end", TextType.TEXT)
        nodes = split_nodes_links([node])
    ...
    def test_image_and_link(self):
        node = TextNode("This has a [link](https://boot.dev) and an ![image](https://boot.dev/image.png) in it", TextType.TEXT)
        nodes = split_nodes_images([node])
        nodes = split_nodes_links(nodes)
        self.assertEqual(len(nodes), 5)
    ...
    def test_only_image(self):
        node = TextNode("![boot.dev logo](https://boot.dev/logo.png)", TextType.TEXT)
        nodes = split_nodes_images([node])
        self.assertEqual(len(nodes), 1)
    ...
    def test_no_space(self):
        node = TextNode("[one](https://first.com)[two](https://second.com)", TextType.TEXT)
        nodes = split_nodes_links([node])
    ...
    def test_link_at_start(self):
        node = TextNode("[first](https://boot.dev) some text", TextType.TEXT)
        nodes = split_nodes_links([node])
        self.assertEqual(len(nodes), 2)
    ...
    def test_link_at_end(self):
        node = TextNode("some text [last](https://boot.dev)", TextType.TEXT)
        nodes = split_nodes_links([node])
        self.assertEqual(len(nodes), 2)
    ...
    def test_malformed_link(self):
        node = TextNode("[partial link(https://boot.dev)", TextType.TEXT)
        nodes = split_nodes_links([node])
        self.assertEqual(len(nodes), 1)
